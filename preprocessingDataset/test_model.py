...
def load_prediction_resources(model_path, class_indices_path):
    """Loads the trained model and class indices map."""
    try:
        print(f"--> Loading model from: {model_path}")
        
        # --- CHANGE ---
        # The .keras format is robust and doesn't need the custom_objects hack.
        # This is the modern, correct way to load the model.
    ...
    if os.path.isdir(input_path):
        ...
        for filename in image_files:
            ...
            if predictions is not None:
                ...
                top_3_preds = [(class_labels[i], predictions[i]) for i in top_3_indices]
                
                print(f"  - {filename}: {top_3_preds[0][0]} ({top_3_preds[0][1]:.2%})")
                report_data.append([filename, top_3_preds[0][0], f"{top_3_preds[0][1]:.2%}"])
        ...
        if predictions is not None:
            top_3_indices = predictions.argsort()[-3:][::-1]
            top_predictions = [(class_labels[i], predictions[i]) for i in top_3_indices]
...
def main():
    parser = argparse.ArgumentParser(description="Cat Breed Classifier - Advanced Tester")
    parser.add_argument("input_path", type=str, help="Path to a single image or a directory of images.")
    
    # --- CHANGE ---
    # Updated the default model path to the .keras format
    parser.add_argument("--model", type=str, default="cat_breed_mobilenetv3_final.keras", help="Path to the trained .keras model file.")
    # --------------

    parser.add_argument("--labels", type=str, default="class_indices.json", help="Path to the class indices JSON file.")
    ...
    parser.add_argument("--output_csv", type=str, default="report.csv", help="Filename for the CSV report when processing a directory.")
