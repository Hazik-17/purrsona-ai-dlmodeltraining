...
# === 2. Configuration (FINAL EXPERIMENT) ===
# --- CHANGE: Point to the binary dataset ---
DATA_DIR = '../cat_vs_not_cat_dataset'
IMG_SIZE = (224, 224)
# --- CHANGE: Use the proven BATCH_SIZE for best generalization ---
BATCH_SIZE = 32
# ---
EPOCHS_PHASE1 = 15 # Binary is an easier task, may not need as many epochs
EPOCHS_PHASE2 = 25
LR_PHASE1 = 1e-3
LR_PHASE2 = 8e-6     # Our champion fine-tuning LR
EARLYSTOP_PATIENCE = 10
# --- CHANGE: Label smoothing is not typically used with standard binary_crossentropy ---
# We will rely on Dropout and Augmentation for regularization.
# LABEL_SMOOTHING = 0.1 
DROPOUT_RATE = 0.5
DENSE_UNITS = 256
UNFREEZE_LAYERS = 50
...
num_classes = len(class_names) # This will be 2
class_indices = {name: i for i, name in enumerate(class_names)}
print(f"\nFound {num_classes} classes: {class_indices}")

# --- CHANGE: Save to the correct indices file ---
print("\n💾 Saving class indices to non_cat_class_indices.json")
...
model = Sequential([
    ...
    # --- CHANGE: Binary classification output layer ---
...
print("\n🔁 Starting Phase 1: Train top layers only (EfficientNetV2B0 - Gatekeeper)")
model.compile(
    optimizer=Adam(learning_rate=LR_PHASE1),
    # --- CHANGE: Binary loss function ---
...
plt.savefig('phase_1_training_plot_gatekeeper.png'); print("\nSaved Phase 1 plot as 'phase_1_training_plot_gatekeeper.png'")

# === 6. Phase 2: Fine-tune last 50 layers ===
print(f"\n🔁 Starting Phase 2: Fine-tuning last {UNFREEZE_LAYERS} layers (EfficientNetV2B0 - Gatekeeper)")
...
plt.savefig('phase_2_training_plot_gatekeeper.png'); print("\nSaved Phase 2 plot as 'phase_2_training_plot_gatekeeper.png'")

# === 7. Final Evaluation on Test Set ===
print("\n📊 Final evaluation on test set (EfficientNetV2B0 - Gatekeeper):")
...
# --- CHANGE: Convert probabilities to binary 0 or 1 ---
y_pred = np.round(y_pred_probs).astype(int).flatten()

y_true = []
for images, labels in test_ds.take(test_steps):
    y_true.append(labels.numpy()) # Labels are already 0 or 1
y_true = np.concatenate(y_true).flatten().astype(int)
...
# --- CHANGE: Save to the correct model file ---
model.save('cat_vs_not_cat.keras'); print("\n💾 Model saved as 'cat_vs_not_cat.keras'")
