...
# === 2. Configuration (Aggressive Regularization) ===
DATA_DIR = '../../output'
IMG_SIZE = (224, 224) 
BATCH_SIZE = 64
EPOCHS_PHASE1 = 20
...
# --- CHANGES for Hypertuning 9.0 ---
LABEL_SMOOTHING = 0.2   # Was 0.1. More regularization.
DROPOUT_RATE = 0.6      # Was 0.5. More regularization.
DENSE_UNITS = 512       # Was 256. More capacity for dropout to act on.
# ---
UNFREEZE_LAYERS = 50
...
model = Sequential([
    ...
    # --- CHANGES for Hypertuning 9.0 ---
    Dense(DENSE_UNITS, activation='relu', kernel_regularizer=l2(0.001)), # Now 512
    BatchNormalization(),
    Dropout(DROPOUT_RATE), # Now 0.6
    # ---
...
model.compile(
    ...
    # --- CHANGES for Hypertuning 9.0 ---
    loss=tf.keras.losses.CategoricalCrossentropy(label_smoothing=LABEL_SMOOTHING), # Now 0.2
    # ---
    metrics=['accuracy']
)
...
plt.savefig('phase_1_training_plot_large_v2.png'); print("\nSaved Phase 1 plot as 'phase_1_training_plot_large_v2.png'")
...
plt.savefig('phase_2_training_plot_large_v2.png'); print("\nSaved Phase 2 plot as 'phase_2_training_plot_large_v2.png'")
...
model.save('breed_expert_mobilenetv3_large_v2.keras'); print("\n💾 Model saved as 'breed_expert_mobilenetv3_large_v2.keras'")
